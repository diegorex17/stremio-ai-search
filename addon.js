...
async function getAIRecommendations(query, type, geminiKey, config) {
  ...
  try {
    ...
    const tools = [new GoogleSearch()];
    const model = genAI.getGenerativeModel({ model: geminiModel, tools: tools });
    ...
    promptText = promptText.concat([
      ...
      "- If this query is for movies from a specific franchise (like 'Mission Impossible movies, James Bond movies', Marvel, DC), list the official entries in that franchise in chronological order.",
